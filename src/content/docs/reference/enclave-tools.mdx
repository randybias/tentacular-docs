...
| `add_members` | string[] | No | OIDC emails to add to the current member list (CSV format). |
| `remove_members` | string[] | No | OIDC emails to remove from the current member list (CSV format). When members are removed, their tentacles are automatically transferred to the enclave owner. |
| `new_owner` | string | No | New owner email (must be a current member). Updates `tentacular.io/enclave-owner`. |
...
| `new_mode` | string | No | New permission mode (9-char rwx string, e.g. `rwxrwx---`). Updates `tentacular.io/mode`. |
| `quota_preset` | string | No | New quota preset. Applies updated resource quota to the namespace. |

### Ownership Transfer on Removal

When `remove_members` is specified, the MCP server automatically transfers all tentacles owned by the removed members to the enclave owner. The response includes a `transfers` array detailing each transfer.
...
```json
{
  "name": "marketing-automations",
  "updated": ["members"],
  "enclave": { "owner": "alice@company.com", "members": ["bob@company.com"], "mode": "rwxrwx---", "status": "active" },
  "transfers": [
    { "tentacle_name": "video-analyzer", "from_owner": "carol@company.com", "to_owner": "alice@company.com", "success": true },
    { "tentacle_name": "data-pipeline", "from_owner": "carol@company.com", "to_owner": "alice@company.com", "success": false, "error": "conflict" }
  ]
}
```
...
# Remove a member (tentacles transfer to enclave owner)
tntc enclave sync marketing-automations --remove-member carol@company.com

# Transfer ownership
tntc enclave sync marketing-automations --new-owner bob@company.com

# Change permission mode
tntc enclave sync marketing-automations --new-mode rwxrwxr--
