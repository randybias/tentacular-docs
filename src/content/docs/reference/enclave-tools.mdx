...
When `remove_members` is specified, the MCP server automatically transfers all tentacles owned by the removed members to the enclave owner. The response includes `transfer_count` and `failed_transfers` fields.
