...
| `sizing` | string | No | Resource quota preset: `"small"`, `"medium"`, `"large"`. Default: `"medium"`. |
...
| `channel_name` | string | No | Updated display name (after Slack channel rename). Does not change the K8s namespace name. |
| `status` | string | No | New enclave status: `"active"` or `"frozen"`. |
| `mode` | string | No | New permission preset or mode string. |
| `sizing` | string | No | New sizing preset. Applies updated resource quota to the namespace. |
