...
| **The Kraken** | A Slack bot that provides a natural-language conversational interface to Tentacular. Uses the Claude Agent SDK in-process (no Docker containers) and connects to the MCP server for cluster operations. Deployed as a single Kubernetes pod. See the [deployment guide](/tentacular-docs/guides/thekraken-slack-bot/). |
